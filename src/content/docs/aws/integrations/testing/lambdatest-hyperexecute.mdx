...
```bash
{/* hyperexecute --user '<user-name>' --key '<HE-key>' --config he.yaml */} {/* mdx-disabled */}
```

{/* Swap `<user-name>` and `<HE-key>` with your HyperExecute username and access key. */} {/* mdx-disabled */}
